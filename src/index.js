...
<body>
  <div class="card">
    ...
    <div id="g_id_onload" data-client_id="${googleClientId}" data-context="signin" data-ux_mode="popup" data-callback="setSession" data-auto_prompt="false"></div>
  ...
  <script>
    function setSession() {
      document.cookie = "landingnl_session=active; path=/; max-age=2592000; Secure; SameSite=Strict";
      location.href = localStorage.getItem('landingnl_age') ? "/dashboard" : "/onboarding";
    }
    document.addEventListener('DOMContentLoaded', function() {
...
<body>
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      document.getElementById('btn-ob-finish').onclick = function() {
...
<body>
  <div class="container">
    ...
    <div class="savings-banner">
      <div>
        ...
        <div style="font-size:1.4rem; font-weight:800; color:#fff;" id="total-savings-text">€1.560+ / Yıl</div>
      </div>
  ...
  <script>
    ...
    function renderProfileGrid() {
      ...
      grid.appendChild(createSubCard(isEU ? 'tag-mint' : 'tag-amber', isEU ? 'Sınırsız Çalışma' : 'Max 16 Saat/Hafta', isEU ? 'TWV İzni Gerekmez' : 'İşveren TWV İzni Almalı', isEU ? 'AB vatandaşları için tam zamanlı serbest çalışma.' : 'Haziran-Ağustos döneminde tam zamanlı serbest.'));
    ...
    function renderAllowances() {
      ...
      grid.appendChild(createSubCard(
        ...
        'Aylık ~€120+ Karşılıksız İade',
        'Hollanda Temel Sağlık Sigortası (Basiszorgverzekering) yaptıran veya çalışan öğrencilerin hakkıdır.',
    ...
    function renderInsuranceTree() {
      ...
      if (!isWorking) {
        ...
        container.appendChild(createSubCard('tag-mint', '💶 Sigorta Maliyetin', 'Aylık €0 Ek Maliyet', 'Mevcut özel/öğrenci sigortanızla hastane ve acil durum erişiminiz geçerlidir.'));
        ...
        container.appendChild(createSubCard('tag-mint', '🚨 ZORUNLU DEĞİŞİM', 'Basiszorgverzekering Yapılmalı', 'Resmi maaşlı bir işte çalıştığınız için Aon/EHIC geçersizleşir. Zilveren Kruis / Loonzorg vb. zorunlu temel sigorta başlatılmalıdır (~€135-150/ay).', 'Temel Sağlık Sigortası başvuru paneline yönlendiriliyorsunuz.'));
        container.appendChild(createSubCard('tag-purple', '💶 DEVLET DESTEĞİ', 'Aylık ~€120+ Zorgtoeslag İadesi', 'Çalıştığınız için ödediğiniz sigorta priminin büyük kısmını devletten geri alma hakkınız doğar. DigiD ile Belastingdienst üzerinden başvurulur.', 'Zorgtoeslag başvuru sayfasına yönlendiriliyorsunuz.'));
    ...
    function renderSwaps() {
      ...
      for (var i = 0; i < allPosts.length; i++) {
        ...
        div.innerHTML = '<div><span class="swap-tag ' + (post.tagClass || 'tag-mint') + '">' + post.cat + '</span><h4 style="margin:2px 0; font-size:0.9rem; color:#fff;">' + post.title + '</h4><p style="margin:0; font-size:0.78rem; color:var(--muted);">Karşılığında: <strong style="color:#cbd5e1;">' + post.offer + '</strong></p></div>';
    ...
    window.addEventListener('DOMContentLoaded', function() {
      ...
      if (btnReset) {
        btnReset.onclick = function() {
          Store.clear();
          document.cookie = "landingnl_session=; path=/; expires=Thu, 01 Jan 1970 00:00:01 GMT;";
          location.href = "/";
        };
      }
